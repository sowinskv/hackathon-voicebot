  ...
  if (!botConfig?.id) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-white">
        <History className="w-16 h-16 text-white/300 mb-4" />
        <h4 className="text-lg font-medium text-white/900 mb-2">
          No Bot Configuration
        </h4>
        <p className="text-white/600 max-w-md">
      ...
      <div className="border-b border-gray-200 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <History className="w-5 h-5 text-white/600" />
            <div>
              <h3 className="font-semibold text-white/900">Version History</h3>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {isLoading ? (
          <div className="flex items-center justify-center h-64">
            <Loader className="w-8 h-8 text-white/400 animate-spin" />
          </div>
        ) : versions.length === 0 ? (
          <div className="text-center py-12 text-white/500">
            <FileText className="w-12 h-12 mx-auto mb-3 text-white/300" />
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <h4 className="font-semibold text-white/900">
                    ...
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="text-white/500">Nodes:</span>{' '}
                          <span className="font-medium text-white/900">
                            {version.flow.nodes.length}
                          </span>
                        </div>
                        <div>
                          <span className="text-white/500">Edges:</span>{' '}
                          <span className="font-medium text-white/900">
                            {version.flow.edges.length}
                          </span>
                        </div>
                        <div>
                          <span className="text-white/500">Fields:</span>{' '}
                          <span className="font-medium text-white/900">
                    ...
                    {version.requiredFields.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs text-white/500 mb-1">Required Fields:</p>
                        <div className="flex flex-wrap gap-1">
                          {version.requiredFields.map((field, idx) => (
                            <span
                              key={idx}
                              className="px-2 py-0.5 bg-gray-100 text-white/700 text-xs rounded"
                            >
                              {field.name}
                            </span>
                          ))}
